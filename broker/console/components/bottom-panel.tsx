"use client"

import { useState } from "react"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { ScrollArea } from "@/components/ui/scroll-area"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import {
  AlertCircle,
  AlertTriangle,
  Info,
  ChevronRight,
  ChevronDown,
  ChevronUp,
  Upload,
  CheckCircle2,
  FileEdit,
} from "lucide-react"
import type { ValidationIssue, PendingChange } from "@/lib/types"
import { cn } from "@/lib/utils"

interface BottomPanelProps {
  issues: ValidationIssue[]
  pendingChanges: PendingChange[]
  validationStatus: "idle" | "validating" | "valid" | "invalid"
  onNavigateToIssue: (nodeId: string) => void
  onValidate: () => void
  onDeploy: () => void
}

export function BottomPanel({
  issues,
  pendingChanges,
  validationStatus,
  onNavigateToIssue,
  onValidate,
  onDeploy,
}: BottomPanelProps) {
  const [isCollapsed, setIsCollapsed] = useState(false)
  const [activeTab, setActiveTab] = useState("problems")

  const errors = issues.filter((i) => i.severity === "error")
  const warnings = issues.filter((i) => i.severity === "warning")
  const infos = issues.filter((i) => i.severity === "info")

  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      case "error":
        return <AlertCircle className="h-4 w-4 text-red-500" />
      case "warning":
        return <AlertTriangle className="h-4 w-4 text-yellow-500" />
      case "info":
        return <Info className="h-4 w-4 text-blue-500" />
      default:
        return null
    }
  }

  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case "error":
        return "text-red-500"
      case "warning":
        return "text-yellow-500"
      case "info":
        return "text-blue-500"
      default:
        return "text-muted-foreground"
    }
  }

  const getCategoryBadge = (category: string) => {
    const variants: Record<string, string> = {
      contract: "bg-purple-500/10 text-purple-500",
      dependency: "bg-orange-500/10 text-orange-500",
      configuration: "bg-blue-500/10 text-blue-500",
      status: "bg-cyan-500/10 text-cyan-500",
    }
    return variants[category] || "bg-muted text-muted-foreground"
  }

  const getChangeTypeColor = (type: string) => {
    switch (type) {
      case "create":
        return "text-green-500"
      case "update":
        return "text-yellow-500"
      case "delete":
        return "text-red-500"
      default:
        return "text-muted-foreground"
    }
  }

  return (
    <div
      className={cn("flex flex-col border-t border-border bg-card/50 transition-all", isCollapsed ? "h-10" : "h-64")}
    >
      {/* Header with tabs and collapse button */}
      <div className="flex items-center justify-between border-b border-border bg-card/80 px-4 py-2 backdrop-blur-sm">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
          <TabsList className="h-8 bg-transparent p-0">
            <TabsTrigger value="problems" className="h-7 gap-2 px-3 text-xs data-[state=active]:bg-accent">
              <AlertCircle className="h-3 w-3" />
              Problems
              {issues.length > 0 && (
                <Badge variant="outline" className="h-4 px-1 text-[10px]">
                  {issues.length}
                </Badge>
              )}
            </TabsTrigger>
            <TabsTrigger value="changes" className="h-7 gap-2 px-3 text-xs data-[state=active]:bg-accent">
              <FileEdit className="h-3 w-3" />
              Changes
              {pendingChanges.length > 0 && (
                <Badge variant="outline" className="h-4 bg-warning/10 px-1 text-[10px] text-warning">
                  {pendingChanges.length}
                </Badge>
              )}
            </TabsTrigger>
          </TabsList>
        </Tabs>

        <Button
          variant="ghost"
          size="sm"
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="h-7 w-7 p-0 text-muted-foreground hover:text-foreground"
        >
          {isCollapsed ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
        </Button>
      </div>

      {/* Tab content */}
      {!isCollapsed && (
        <Tabs value={activeTab} className="flex flex-1 flex-col overflow-hidden">
          {/* Problems Tab */}
          <TabsContent value="problems" className="m-0 flex-1 overflow-hidden">
            <ScrollArea className="h-full">
              <div className="p-2">
                {issues.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    <div className="mb-2 rounded-full bg-green-500/10 p-3">
                      <CheckCircle2 className="h-6 w-6 text-green-500" />
                    </div>
                    <p className="text-sm font-medium text-foreground">No problems detected</p>
                    <p className="text-xs text-muted-foreground">All contracts and dependencies are resolved</p>
                  </div>
                ) : (
                  <div className="space-y-1">
                    {issues.map((issue) => (
                      <button
                        key={issue.id}
                        onClick={() => onNavigateToIssue(issue.nodeId)}
                        className="group flex w-full items-start gap-3 rounded-md border border-transparent px-3 py-2 text-left transition-colors hover:border-border hover:bg-accent/50"
                      >
                        <div className="mt-0.5">{getSeverityIcon(issue.severity)}</div>
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            <span className={cn("text-sm font-medium", getSeverityColor(issue.severity))}>
                              {issue.message}
                            </span>
                            <Badge
                              variant="outline"
                              className={cn("h-4 px-1 text-[10px]", getCategoryBadge(issue.category))}
                            >
                              {issue.category}
                            </Badge>
                          </div>
                          <p className="font-mono text-xs text-muted-foreground">{issue.path}</p>
                        </div>
                        <ChevronRight className="mt-1 h-4 w-4 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100" />
                      </button>
                    ))}
                  </div>
                )}
              </div>
            </ScrollArea>
          </TabsContent>

          {/* Changes Tab */}
          <TabsContent value="changes" className="m-0 flex flex-1 flex-col overflow-hidden">
            <ScrollArea className="flex-1">
              <div className="p-2">
                {pendingChanges.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    <div className="mb-2 rounded-full bg-muted p-3">
                      <FileEdit className="h-6 w-6 text-muted-foreground" />
                    </div>
                    <p className="text-sm font-medium text-foreground">No pending changes</p>
                    <p className="text-xs text-muted-foreground">Make changes to see them here</p>
                  </div>
                ) : (
                  <div className="space-y-1">
                    {pendingChanges.map((change) => (
                      <div
                        key={change.id}
                        className="flex items-start gap-3 rounded-md border border-border bg-card/50 px-3 py-2"
                      >
                        <Badge
                          variant="outline"
                          className={cn("mt-0.5 h-5 px-2 text-[10px]", getChangeTypeColor(change.type))}
                        >
                          {change.type}
                        </Badge>
                        <div className="flex-1 space-y-1">
                          <p className="text-sm font-medium text-foreground">{change.data.name}</p>
                          <p className="font-mono text-xs text-muted-foreground">
                            {change.data.type} • {change.timestamp.toLocaleTimeString()}
                          </p>
                        </div>
                      </div>
                    ))}
                  </div>
                )}
              </div>
            </ScrollArea>

            {/* Actions footer for Changes tab */}
            {pendingChanges.length > 0 && (
              <div className="border-t border-border bg-card/80 p-3 backdrop-blur-sm">
                {validationStatus !== "idle" && (
                  <div className="mb-3 flex items-center gap-2 text-sm">
                    {validationStatus === "validating" && (
                      <>
                        <div className="h-4 w-4 animate-spin rounded-full border-2 border-primary border-t-transparent" />
                        <span className="text-muted-foreground">Validating...</span>
                      </>
                    )}
                    {validationStatus === "valid" && (
                      <>
                        <CheckCircle2 className="h-4 w-4 text-green-500" />
                        <span className="text-green-500">Valid</span>
                      </>
                    )}
                    {validationStatus === "invalid" && (
                      <>
                        <AlertCircle className="h-4 w-4 text-red-500" />
                        <span className="text-red-500">Invalid</span>
                      </>
                    )}
                  </div>
                )}

                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="flex-1 bg-transparent"
                    onClick={onValidate}
                    disabled={validationStatus === "validating"}
                  >
                    Validate
                  </Button>
                  <Button size="sm" className="flex-1" onClick={onDeploy} disabled={validationStatus !== "valid"}>
                    <Upload className="mr-2 h-4 w-4" />
                    Push
                  </Button>
                </div>
              </div>
            )}
          </TabsContent>
        </Tabs>
      )}
    </div>
  )
}
